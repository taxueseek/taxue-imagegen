...
- `paper_white._box_mean` 的窗口是「以像素为起点向右下延伸 2r」而非 docstring 所称的居中
  r 邻域（脉冲响应实测支撑 `[i, i+2r]`）。对成品影响约 1e-4 量级，但边界/渐变处的局部均值
  取样位置系统性偏移 16 px——修它要重跑 A/B 比对，不塞进本轮。
- `postcheck` 在类型 E 下对同一张图解码两次（`metrics()` 与 `grid_metrics()` 各一次
...
- 两条**实测过、且已验证改法 bit-exact、但按性价比本轮不动**的资源项（走查时逐条量过）：
  ...
  **不动的理由**：两者都是「疑难/排障才跑」的脚本，不在日常出图路径上；而裁窗必须
  留够 `ksize//2+1` 的上下文行才 bit-exact，本轮没有余量做逐像素比对。
  同样可排除的伪优化（都已实测无收益）：`np.load` 每次 0.09 ms、`medianBlur(gray,3)`
  0.4 ms、PNG 压缩等级调低反而更慢。
