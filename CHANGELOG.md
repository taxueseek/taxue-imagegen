...
- **噪声底被量出来了**：6 次运行的全部分歧只有 1 条（类别：**参考一张已有图 + 同时要一张新的海报提示词**，1/6 组给出相反判定）。即单次运行逐条噪声约 1/216，实际含义是「每 36 条约有 1 条会翻」。
  第一轮那句「误触发 1 条 → precision 0.90」落在噪声里，**第一轮的分辨力不足以支持比较**，
  这一轮的结论覆盖它。
- **标签存疑的那条**（类别：**点名由另一家插件执行出图**）——
...
**纪律（写了就要守）**：**取自会话的语料一律不引原文，连片段也不引**（自行构造的样例可引，
它们不是语料）。本节第一稿就把两条真实消息逐字抄进了这个文件，而 CHANGELOG 是公开仓的一部分；
更糟的是我第一次只改了发布仓的副本，随后 `sync_release.sh` 从真源整份覆盖回去，把修复冲掉——
**真源才是唯一改得动的地方**。仓里的隐私门禁只查本机绝对路径，**查不出这类「用户原话」泄漏**，
所以这条靠人守，写在这里当提醒。
