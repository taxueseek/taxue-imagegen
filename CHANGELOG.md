...
> 同日第三批：一次**量化驱动的性能与常驻面**优化轮。两条主线——把默认验收路径里最大
> 的一笔纯 Python 开销换成 C 层算子（用 304 张真实成品验证逐位一致），以及把「每轮都要
> 付」的常驻面按它自己的分层原则再压一轮。顺带修掉五个**一类**缺陷，其中一个是守卫
> 写了但从未生效。断言 288 → 317，Python 3.9 / 3.13 / 3.14 三套解释器下全绿。

### 性能

- **`measure.metrics` 改用 PIL 的 C 层算子**（数量级最直观的一处）。原实现把 256 宽的
  缩略图展开成 **9.8 万个 RGB 三元组**，再对同一批像素做**十几趟** `min()`/`max()`
  （white% / near% / base / paper% / zone 三通道 / sat 各一趟，顶部区还把
  `get_flattened_data()` 跑了两遍）。现在 min/max 通道由 `ImageChops.darker/lighter`
  给出、分布由直方图给出、带掩膜均值由 `ImageStat.Stat(im, mask)` 给出，全在 C 层完成。
  **1024×1536 单次 126 ms → 31 ms**；在 304 张已归档成品上与原实现**逐字段比对，最大
  绝对误差 0.0**（不是"接近"，是逐位一致）。默认验收路径随之提速：
  `postcheck --wm off` 233.6 → 144.2 ms（**−38%**）、默认 `--wm auto` 324.0 → 238.4 ms（**−26%**）。
- 成本结构实测（1024×1536）：Python 启动 ~10 ms / PIL import ~30 ms / **cv2+numpy import
  ~90 ms**（只在 `--wm auto` 时付，是水印能力的固有代价）/ `measure.metrics` **126 ms**
  ← 唯一一笔可消除的纯开销 / 水印识别（inpaint + 模板）~15 ms / 文字带编码 ~26 ms。

### 常驻面（每轮对话都付的那部分）

- **SKILL.md 33,234 → 30,056 B（−9.6%）；cl100k 13,461 → 12,222 tokens（−1,239，−9.2%）。**
- **即梦 2K 尺寸表从 SKILL.md 下沉到 `references/jimeng-env.md` #9.0。** 原先那张表在
  常驻面，而 `jimeng-env.md` 第 20 / 88 行反倒**指回** `SKILL.md` 的那张表——细节留在
  每轮都付的那一面、按需面反过来引用它，正是本文件分层原则要防的方向。现在表与用法
  同处一页，SKILL.md 只留一行指针（`jimeng.py --sizes` 仍是可执行真源）。
- `#6` / `#9` 的正文压成指针（完整版早就在 `cloud-postprocess.md` / `jimeng-env.md`）；
  `#8` 索引压掉 1.1 KB（保留全部文件名与"何时用哪个"，合并旧版/实验版去水印的九行）；
  `#5` 与验收子技能重复的散文压缩为三条处置入口。
- **符号统一**：`#` 取代 `§`（176 处），全角圆括号改半角（1,247 对，**仅散文**）。
  这是先量后改的：`§` 在 cl100k / o200k 里都是 1 token，全仓替换**只值 10 tokens**，
  真实收益来自圆括号；`「」` 与 `·` 实测会**变差**（o200k +31 / cl100k +25），故不动。
  `【】`（C/E 槽位定界符，代码解析）、`❌⚠️✅`（脚本输出前缀，门禁断言）、
  `α σ κ̂ R² ≥ ≤` 等数学记号一律保留——它们是承载语义的，ASCII 化既省不了 token
  也丢精度。
- 改符号时**围栏代码块一律不碰**（模板即已验证资产），并**整份排除 `crowd-themes.md`**
  （它的章节正文本身就是类型 B 的提示词）。改完用五路出稿指纹验证：
  `fill_meta A/B/C/E` + `jimeng A` 的 stdout **逐字节不变**。

### 修复（五个「一类」+ 若干单点）

1. **`dewm_v11` 的「1−α 过小」守卫恒为 False，从未生效过。** `alpha` 先被
   `np.clip(..., ALPHA_GUARD)` 夹住，下一行才写 `ill_cond = alpha > ALPHA_GUARD`——
   v8 / v10 用的是裁剪前的 `alpha_raw`，只有 v11 掉了这一半判据。后果是 α 被夹到 0.92
   （1−α = 0.08，反解误差放大 12.5 倍）的像素不再退回 inpaint 背景，而是照直写进产物。
2. **α 模板框没夹进画面 → 畸变尺寸把整条验收打崩。** 模板框按宽度等比缩放
   （1024 宽时 224×106），画面比它矮或比它窄时 `H − bh` 变负，调用方拿负偏移去切片：
   实测 1600×100 报 `ValueError: could not broadcast input array from shape (166,350) …`
   （postcheck 判成工具错误）、1×1 报 `cv2.error`（`cv2.resize` 收到 0×0）。
   现在框被夹进画面；**在所有声明画幅上是恒等变换**（锚点逐位不变），退化画幅则退化成
   贴边小框、检测自然判「无水印」，不再抛异常。
3. **`sync_release.sh` 会把「已验证风格的提示词原文」同步进发布仓。** `EXCLUDES` 里写的是
   `--exclude='_prompts'`，而 rsync 的模式**不是前缀匹配**，匹配不到 `_prompts_archive/`；
   `_calib_cache/` 与 `wm_alpha_1024.npz.bak-*`（模板的本地备份）同样没列。三者都在
   `.gitignore` 里（= 明确"只留本机"）。已补；并新增门禁把「`.gitignore` 锚定在同步树上的
   模式」与 `EXCLUDES` 逐条对齐——**用 .gitignore 而不是"实际存在的被忽略文件"做输入**，
   因为干净检出里那些目录本就不存在，按存在性判定会空转或假红。
4. **`dewm2` / `rmwm` 整批全失败仍报成功。** 逐图 try/except 只打印、**从不设退出码**：
   9 张全部读不了也是一句"共处理 0 张" + 退出码 0。现收敛为退出码 4（工具/输入错误，
   与 postcheck 的 4 同义，**不是 blocker**）。
5. **`rmwm_light` 读不了一张图 → traceback + 退出码 1**，而 1 在本技能约定里是「blocker」：
   一张坏图把整批判成"图有毛病"（正是 postcheck 修过的那一类）。现逐图兜住异常并归入退出码 4。

**单点**：7 个脚本共 9 处把「用法/输入错误」报成退出码 1（`sys.exit("msg")` 恰好是 1，
会被读成 blocker）；`dewm_v11` / `dewm_v10` / `dewm_v9` docstring 引用的**是被推翻的常数**
（融合斜坡 0.05/0.40 实为 0.02/0.12、尺度集 5 个实为 8 个、`flatness` 说"四周"实为三侧）；
SKILL.md 说 CHANGELOG「约 49 KB」实为约 107 KB；`crowd-themes.md` 两张实测表仍把**已撤口径**
（white% 30–65 / sat 20–60）当"阈值（类型 B）"列出，与 `#5` 直接冲突。

### 重构与死代码

- **`load_template` 四份副本收进 `dewm_io`**（dewm / v7 / v8 / v9 各一份，逐字等价——
  npz 里 alpha 本就是 float32，只有 dewm.py 多一行尺寸警告）。全族（v10–v13、audit_wm、
  wm_auto、pick_wm、metric_flat 与各 bench）都经它取模板，因此上面第 2 条边界修复
  **只需改一处**。`dewm.py` 保留尺寸警告当薄包装（若一并搬进共享实现，1536x864 这类
  **合法**画幅每次调用都会刷一行 stderr——那是误报）。
- 删 `measure.grid_metrics` 里 20 行**不可达**的重复判据块；删 `dewm_v9._remove_at`
  连续重复的 3 行；`pick_wm` 的 `ambiguous_pair` 只算一次（原先判一次、分支体内再算一次）；
  删 `paper_white` 的死分支（`dst` 恒以 `.png` 结尾）、`_env.need()` 与
  `bench_dewm_align.inject_shift()`（全仓无调用）；`tests/test_docs_gates.py` 的
  `test_referenced_files_exist` 此前**排在文件头之前**（shebang / coding cookie /
  模块 docstring 与全部 import 都在它下面第 48 行起，于是 `__doc__` 是 None，而没有任何
  一条门禁会红）；`tests/test_fill_meta.py` 一处赋值立刻被下行覆盖。

### 新增门禁（288 → 317 条断言）

- `measure.metrics` 与「逐像素朴素实现」逐字段等价（含全黑 `base=0` 与全白 `base=255`
  两个边界——正是直方图切片会踩负索引 / 越界的地方），期望值一律取自参照实现而非直觉；
- `dewm_v11` 的裁剪前 α 守卫必须能触发；
- α 模板框必须永远落在画面内，且**声明画幅上与"不夹取"逐位一致**（防"修边界改坏正常路径"）；
- 用法错误 → 退出码 2（5 个脚本）、整批失败 → 退出码 4（2 个脚本）；
- `.gitignore` 锚定在同步树上的模式必须都被 `EXCLUDES` 覆盖；
- 每个 `.py` 的首条语句必须是模块 docstring（用 `ast` 判，零误报）。

### 验证

- 回归 **317/317**；总门禁在 **Python 3.9 / 3.13 / 3.14** 三套解释器下全绿。
- **功能矩阵 40/40**：7 个声明尺寸 × 5 个类型 × 水印全族（wm_auto / dewm_v10 / pick_wm /
  audit_wm / metric_flat / paper_white / dewm_imprint / rmwm / rmwm_light / dewm2）＋
  fill_meta A/B/C/E、jimeng、explore、preflight 五类型。
- **健壮性矩阵 31/31**：0 字节 / 截断 PNG / 1×1 / 1600×100 / 非图片 / 中文+空格文件名 /
  RGBA / P 调色板 / 16 位灰 / 4000×4000；换 cwd 运行；缺 α 模板；五个脚本缺依赖时都给出
  退出码 2 + 人话（无 traceback）；`TAXUE_LOG` 未设时本地日志正常写。
- 能力无损的证据：304 张真实成品上 `metrics` 逐字段零误差；声明画幅的模板锚点与改前逐位相同。
...
### 资产盘点（本次实测，结论见 `_research/asset-inventory-2026-09-23.md`）

一轮台账驱动的全量盘点（70 条 `runs.csv` + 分层清点）。**技能本体健康，问题在技能边界之外**：

- **发现一整片未归位的已验证资产**：另有一套「横版 3:2 限色平印人物海报」母版
  （骨架逐字固定 + 11 个槽位）与 **11 个实测 pass 的风格**，连同它们的成品提示词原文，
  全躺在出稿批次目录里，技能内一份都没收。**这批风格里 §六·戊 登记的那 13 个中的 11 个，
  字段其实齐全**（风格名 / 英文主标题 / 中英短句 / 三色 / 表达意图），
  所以那节写的「多数取不回来」是错的——**它们可以零积分恢复**。
- **顺带纠正 §六·戊 的处置方向**：那批是**横版**骨架的产物，而 §六 / §六·乙 是**竖版**表，
  照原建议填进去会得到一张挂不上骨架的表。
- **修掉一处仓库污染**：仓库根有一份 2 行的 `runs.csv`（内容是一次冒烟记录，图已不存在），
  却被 git 跟踪，而真台账在 `scripts/logs/runs.csv`。危险不在于多一个文件，而在于
  文档里一律简称「runs.csv」——**读的人会把冒烟数据当成生产记账，而技能里所有
  「先看台账再决定改什么」的动作都建在这份数据上**。已移出跟踪 + 忽略 + 归档到 `.trash/`。
- 新增门禁 `test_repo_root_whitelist`：`git ls-files` 里位于根目录的条目必须 ∈ 白名单，
  一行挡住 csv / png / 临时 md 混进根目录。已做反向验证（清理前红，清理后 ok）。
  无 git 时显式打印「未校验」，不假装通过。
- **一处自我纠错（原结论写错了，留着免得再犯）**：我先按时间戳把 `text_unverified`
  的 12 行读成「一次批量扫描被记成十几次失败」，并据此写「高频码榜排序不可采信」。
  复核 `file` 列后**结论推翻**：12 行落在 **12 个不同的文件**上，那就是 12 张图各自没过
  逐字校验，**计数没有虚高**；错的是把它当 12 次独立事件去排优先级——其中 9 张同属一个批次
  （同 `note`、同一分钟、同一个根因：九宫格缩略图上的字太小），该被读成**一个待解决问题**。
  真正存在重复计入的是 `grid_diag`（按行 7 / 按图 3 = 2.3x，来自同图重复量测与当时
  尚未修的「同行两个同名码」）。修法与门禁见下一条。
- 预算账：`description` 已用 **93%**（1864 / 2000 B），`SKILL.md` 用 88%。
- 明确**不是**损失的两处（避免下一轮重复排查）：四个实验去水印脚本
  （跨图共识 / αM 反解 / 混合路由 / 平台署名判据）**已全部吸收**进 `dewm_imprint.py`；
  转写进 `pitfalls.md` 的两条教训（黑场走反解会整块残留、判据不能用跨图离散度）也已在。

### 资产恢复（同日执行，本批最大的一笔）

盘点只是把东西找出来，这一节才是把东西拿回来。**零积分，风格库 18 → 29（+61%）**：

- **新建 `references/poster-h-series.md`**（同日第二轮改名为 `poster-horizontal-series.md`，见下）：横版 3:2 限色平印人物海报的**第二支母版**——
  骨架逐字固定（与 `poster-v5.md` 的竖版骨架不通用）、11 个槽位语义、两条跨风格硬规则，
  外加 **11 个已验证风格的完整槽位表**（表 A 主槽位 + 表 B 场景槽位）。
  表 B 的四个字段（`{内容主题}` / `{表达意图}` / `{主体形象}` / `{背景元素}`）是从提示词原文里
  逐条抽回的——**这正是 §六·戊 当初判定「取不回来」的那四项**，实测全部齐全。
- **新建 `references/h-series-prompts.md`**：11 份成品提示词原文，逐字校验与源文件一致
  （脚本比对，非目检）。做成独立文件而不是塞进母版，理由是加载经济学：
  写新风格只需读 12.8 KB 的母版，42.9 KB 的原文只在精确复现时读。
- **`poster-v5.md` §六·己「非 v5.4 骨架的已验证风格」**：给那些骨架不是 v5.4、
  因而进不了 §六/§六·乙/§六·戊 任何一张表的 pass 风格一个家。
  首个入册的是极简东方水墨动态速写（四段式骨架、严格依参考图、画面唯一文字是印章印文）。
- **纠正 §六·戊 的两处错误结论**（原文保留，标注纠正，免得再犯）：
  ① 「主题与短句多数取不回来」是错的；② 「填进 §六/§六·乙」方向也错——
  那是横版骨架的产物，填进竖版表会得到挂不上骨架的图。
- 发布前对这 11+2 份提示词做了身份自检（URL / 本机路径 / 平台名 / 账号 / 项目名，零命中）。

### 防复发：提示词存档进验收流程（`postcheck --prompt`）

上一节把东西拿回来了，这一节让它别再丢一次。**根因是一处分家**：技能的「已验证资产」
= 风格 **加上它的提示词**，可提示词一向由出图流程写在**出稿批次目录**里（不在技能内、
不在任何备份内），而技能只记 verdict。

- `postcheck.py` 新增 `--prompt <提示词.txt>` 与 `--archive-dir`：验收时顺带把这一版原文
  收进 `scripts/_prompts_archive/`（本地、已 gitignore），**按 `--note` 的
  `A/<视觉风格>/<主题>/<主标题>` 落名**——下次盘点 `ls` 一下就能列出所有验证过的风格，
  不必再去翻散落的出稿目录。`--note` 非约定格式时退回按文件名命名，不报错。
- 三条纪律都钉在测试里（`test_prompt_archive`，4 条断言）：① **名字带风格/主题/主标题**
  （不然存档只是换个地方散落，盘点还得靠反推）；② **同一版重复验收幂等**（同内容不产生 `__v2`）；
  ③ **`--no-log` 时不落盘**——与 `runs.csv` 同一条纪律：跑测试/验证不污染生产存档。
  另外「文件不存在」「写不进去」只告警，**不让一个考古动作把验收本身弄失败**。
- **存档不等于入库**：写进 `references/` 风格库、能被下次直接复用，仍是一次有意的动作；
  这一步只保证东西在一个稳定、可检索的地方。这条边界写进了函数 docstring，
  免得下次把「已存档」误当成「已入库」。

### 读数口径：码频次给「按行 / 按图」两个口径（`log_report.py`）

上一节那次自我纠错的直接产物。原实现只给「按行」一个数，于是两种完全不同的东西
长得一模一样：**同一张图被量测多次**（数字虚高）与**同一批里 N 张不同的图各自失败**
（数字是真的）。补上第二个口径就分得开了：

- 码频次表改为 `按行 | 按图 | 码`，比值 >1 才标「← N.Nx 重复」。
  实测立刻分出两类：`text_unverified` 12/12（无重复，真 12 张）、`grid_diag` 7/3（2.3x，真重复）。
- 新增**按 `note` 分批次**小节（行数 / 图数 / verdict 分布）：一个批次 = 一个待解决问题。
  实测 `classic9 scan` 9 行 9 图 pending×9 —— 一个根因，修一次就全好。
  **因此不需要给 `runs.csv` 加批次列**：出图流程本来就在 `note` 里写了批次，
  动表结构（含老机器的迁移路径）是白付的复杂度。
- 新增 `note 空值率`（现 44%）：空 note 的行无法归因到风格/主题，风格库就回流不了。
  这条数字不摆出来，缺口只会靠下次盘点发现。
- 最高频码那句改成先报「按行按图是否一致」，并加一句判读顺序：**先看批次，再看码**。
- 门禁 `test_log_report_dual_count`（4 条断言）：按行真实计入、按图折同图重复、
  批次行列数与图数分开、空值率算得出。**做过两轮反向验证**（把按图口径退化成全 0、
  把批次不再记图数，断言都立刻红）。

### 闭合 C / D 的记账缺口（文档不一致，非设计意图）

台账 70 行全是 `track=A/E`，B/C/D 一条没有。查下来不是「没人用」，是**验收在文档上断了线**：
§5 的指标表**有** 类型 C / 类型 D 的列、`postcheck` 也支持 `--track C|D`，
但 §4 加载协议里 C / D 那两行的「必读」**没写 postcheck 输出**（A/B/E 都写了）。
按文档执行，C/D 的成品就不会进台账，`log_report.py` 永远看不到它们的真实表现。
已给 C / D 两行补上 `postcheck 输出（--track C|D）`，与 A/B/E 一致。
B 那行本来就写了 postcheck 却 0 条 —— 这个仍待查（是没跑过 B 的成品验收，还是跑了没记账，
在拿到第一条 B 记录之前无法区分）。

### 第三支母版入库 + 补回一整类被漏掉的资产（同日第二轮）

上一节说「13 个里剩 1 个提示词不在磁盘上」——**那个说法也是错的，现在 13/13 全齐**。
根因是我第一轮只用 `glob` 遍历了出稿批次的 `_prompts/` 目录，漏掉两类东西：

- **元提示词家族 4 份 / 73 KB**：它们不是「某张图的提示词」，而是**一支母版本身**。
  其中「竖版概念海报」（巨物+巨字+微型参照物）是 `poster-v5.md` 的祖先、
  「手绘动物群像海报」是类型 B 的祖先、「H 系横版限色平印」本轮已入库。
  **找法是搜 WorkBuddy 的 `artifact-index/*.json`**（`present_files` 记录里带原始 uri），
  不是遍历目录——这句写进归档 README，免得下次再用遍历法漏一遍。
- **第三支母版：复古宣传画 · 木刻版画**（宽幅 2.5:1、四色限定、`1536x768` 精确输出）。
  它自带骨架与槽位表，还有 A/B 两版**同构替换**写法（只换主标题/左栏/主景/右栏四项，
  其余逐字相同），台账 `pass`。已纳入 `poster-horizontal-series.md` §六。

**顺带做了一次结构整理**：`poster-h-series.md` → **`poster-horizontal-series.md`**，
因为文件里现在有两支横版母版（§一–§五 H 系 3:2 / §六 复古宣传画 2.5:1），
而「横版海报系列」正是出稿目录里用户自己用的名字。**名字要能被人检索到**——
用 `poster-h-series` 装一支不叫 H 的母版，等于把东西藏起来。

### 新门禁：活文档引用的 .md 必须存在（改名带出来的那条）

上面那次改名**一步制造了 4 处悬空引用**（`poster-v5.md` 2 处、`h-series-prompts.md` 2 处），
而全仓门禁是绿的——因为 `test_surface_layering` 只证明「每个存在的文件都被引用」，
**证不了「被引用的文件都存在」**，方向是反的。补上新门禁 `test_referenced_files_exist`。

这条门禁自己踩了三个坑，值得记下来（都是反向验证抓的，不是想出来的）：

1. **第一次只匹配带前缀的 `references/xxx.md`** → 反向验证时漏判，等于没牙。
   实测活文档里 18 种 `.md` 标记只有 1 种带前缀，多数是裸文件名。
2. **改成只匹配裸文件名** → 又漏了带前缀那种：裸名那条的 lookbehind 排除 `/`，
   而 `references/` 里的 `/` 正好把它挡住，整个匹配不上。
   最终两条正则并行收（带前缀 + 裸名）。
3. **改门禁时把函数插重复了**（`s.index()` 找到的位置与切片边界反向，插到了文件头），
   Python 取后定义 → 新门禁被自己覆盖成旧的。**是仓库已有的
   `test_test_modules_are_wired_exactly_once` 抓出来的**，报错直接点名
   `test_docs_gates.py: ['test_referenced_files_exist']`。这条已存在的门禁第一次
   在真实事故里发挥作用，记一笔。

判据按**存在性**、不写死名单（引用文件 / SKILL.md / CHANGELOG.md / `sub-skills/**/*.md`，
其余 `.md` 即悬空）；范围只扫活文档——`CHANGELOG.md` 里出现旧文件名是**历史记录**，
不该要求它存在（改名这件事本身就是靠它记下来的）。

### 修掉一句源头错话：「没实测」被写成了「没落点」

上面撤回的那条待办，**根子在文档里**：`poster-v5.md` §一·丁 原文写
「扫描切片/结构图/模糊/线条/纹理/符号化 六项**尚无落点**」——而同一节的选项库
逐条给了这 11 条规则的「建立主体的用法」与「能表达的含义」，**那就是落点本身**。
我把「没实测」读成「没落点」，还据此生成了一条「补 6 类模板」的待办，差点去干白活。

已改 §一·丁 的实测状态段：**把「落点」与「实测」两个概念显式分开**
（落点 = 选项库给了写法；实测 = 真出过图并通过），逐项列出「已实测 pass / 已在别处覆盖 /
尚未实测」，并留一句修正说明。**改源头而不是只改报告**——否则下一个人还会照着同一句话走偏。

### 撤回两条前提不成立的待办（复核后）

- ~~「图形规则 6 类无模板落点」~~ → **§一·丁 早就把 11 条规则全给了落点**
  （每条配「建立主体的用法」+「能表达的含义」），还登记了实测状态。
  真正缺的是**另外 8 条的实测记录**，那要真出图（花积分、属用户决策），
  不是「补模板」。原判把「没实测」说成了「没落点」。
- ~~「`pitfalls.md` 加一张坑号索引表置顶」~~ → **已有「按症状速查」**，
  开篇就写「用你看到的现象找行，不要用你猜的根因找行」。那是按**症状**索引，
  比按坑号索引更贴真实入口（人来的时候只知道现象，不知道坑号）。
  再加一张是重复机制，人工表还会与正文漂移。撤掉。
  顺带核对：坑数 37 三处一致（SKILL.md 自述 / pitfalls.md 自述 / 二级标题实际计数），无过期数字。

### 真实验收才发现的事：入库的骨架是「文档版」，照它填过不了自己的预检

前几节都说「资产已收回、零缺」。**直到拿它当黑盒跑一遍真实入口，才发现收错了版本**。

**怎么发现的**：不走自己的测试夹具，改成「按 SKILL.md §4 的路由读文件 → 用 §一 骨架 +
§三 的表机械填一张 → 跑真 `preflight.py`」，工作目录设在 `$HOME`（陌生 cwd）。
结果 **1 阻断**：`❌ [坑14A] 主标题穿插场景缺「只出现一次」显式声明`。

**根因**：出稿目录里 H 系骨架有**两版**——
`meta_prompt_h9.md` §一 的**文档版**（我入库的那版）与 `build_h9.py` 的 `TEMPLATE`
（**11 张实测 pass 的图由它生成**）。两版**槽位名完全相同**，但文档版少三处硬化：

| # | 生产版 | 文档版 | 后果 |
|---|---|---|---|
| 1 | 巨字横带末：英文主标题**只出现一次**，除这一处外任何区域不得再出现它 | 缺 | **坑 14A 阻断** |
| 2 | 纸底**明亮且接近中性、不是浅灰** | 纸底肉眼接近中性 | 坑 9 提醒 |
| 3 | …不出现**刺眼的高饱和色块** | 无高饱和荧光色 | 未触发 |

**修法**：§一 换成生产版骨架（逐字）。复跑：**阻断清零**（只剩坑 16 长度这条已知提醒）。
换骨架后重新组装、在 **3.12.12 / 3.13.12 / 3.14.7 三个解释器 + `$HOME` 陌生 cwd** 下各跑一次，
判定一致。

**教训（写进文件了）**：回收「某母版的骨架」时要收**真正跑出那些图的那一版**，
不是文档里叙述的那一版——文档会漂移，而漂移的代价是「照文档填出来的提示词过不了自己的预检」。

**新门禁 `test_horizontal_skeleton_passes_preflight`** 把这条路径固化：从文件里读骨架与表、
填 02 青花、跑真 `preflight.py`、要求 **0 阻断**（判据是 preflight 自己的退出码，
不是我造的夹具）。反向验证：把骨架退回文档版，断言立刻红，报的就是坑 14A。

### 另一处：复古宣传画的坑 1 是「假阻断」（台账实证）

同一轮验收里，把复古宣传画骨架填好跑 preflight，报
`❌ [坑1] 色相词正向使用：['做旧', '米白']`——而那正是它 §6.1「纸张底色 · 关键项」要的东西。

**台账实证它确实是假阻断**：该风格实测 pass 的那张 `base=115 white%=0.0 R-B=27.8 sat=40.4`，
hint 含 `paper_warm_hint`——**纸底确实显著偏暖，而且这正是这支风格要的效果**。
坑 1 是全文扫描，判的是「提示词里出现色相词」，而本支的语义是「这支风格的纸底就该暖」。

**处置：不改原文**（原文有生产证据，改了就破坏风格）；在 `poster-horizontal-series.md` §6.4
记成「已知假阻断」，并写清「别为它重写关键项——那属于假阻断白扣积分的同类风险（坑 27 家族）」。
要收口应改**规则**（把坑 1 收窄到背景子句 / 给限色平印+旧纸底豁免），
但先要有反例样本证明收窄不会漏掉真泛黄。与待实施里「类型 B 主题库与硬规则 1 的措辞冲突」
是**同一类问题**，一并登记。

顺带量到：源文件两个具体版里那句 `主标题「前进」两个字必须水平并排` 的引号会触发坑 5，
**槽位版骨架没有这个问题**（槽位处无引号），所以从 §6.1 起填比抄具体版干净。

### 横版家族接进路由：它**不走** `fill_meta`（否则静默出错画幅）

加完两支横版母版之后才发现一个路由缺口：`SKILL.md` §3 步 2 写「类型 A/B/C/E 走脚本（默认）」，
而 **`fill_meta.py` 的 A 轨骨架是竖版 2:3**（比例默认就是 `2:3`，11 个槽位也是竖版那套）。
后果很具体：**给横版需求跑 `fill_meta A`，会静默拿到一张竖版提示词**——不报错，画幅就错了。

补法（doc 级，没动代码）：

- §3 步 2 补一句：**横版（H 系 / 复古宣传画）不走 `fill_meta`**，按 `poster-horizontal-series.md`
  手写填骨架，填完照常跑 `preflight.py` 与 `postcheck.py --track A`。
- 横版文件开头加 **§零 怎么用（三步）**，把「选支 → 填骨架 → 验收」写成可照着做的步骤。

写这个「怎么用」时**我自己先写错了一版**，值得记：我按 H 系的约定写了
「结构标签用【】、槽位用 `{}`，两者不会混」——**这句对复古宣传画那支是错的**。
实测校准后两支的括号约定根本不同：

| 支 | 槽位 | 结构标签 |
|---|---|---|
| H 系 | `{视觉风格}` 等 **`{}`** 13 个 | 【三项输入】【版式骨架】等 8 个 |
| 复古宣传画 | **也是【】**：`【主标题】``【左栏文字】``【主景】``【右栏主词】``【右栏副句】` 共 5 个 | 【画幅】【配色】【质感】等 8 个，**同一种括号** |

所以复古宣传画那支的真实陷阱是：**「把所有【】替换掉」会砸掉骨架的结构标签**。
已按两支分别写明，并把陷阱钉进门禁——`test_horizontal_skeleton_passes_preflight`
现在同时验两支：复古那支按 §6.2 只替换 5 个槽位后，**断言 8 个结构标签一个不少**，
且断言的判据前提（「骨架里确实混着标签」）本身也被断言——**将来若不再混用，门禁会提示删掉那条警告**，
而不是留一条过期警告在原地。

### 一个烂到会骗过门禁的区块：§6.2 表只有表头（而门禁「通过」）

上面给横版那支加的三条断言，反向验证时**一条都没红**。查下来是两件事叠在一起：

1. **我发布的 §6.2 槽位表是畸形的**：生成那一节时把源文件自带的 `### 槽位说明` 小标题
   也一起塞了进来，于是出现**两套表头**，5 行数据行要么被挤掉、要么丢了表格语法。
   文档让读者「按 §6.2 的表替换那 5 个槽位」，而表里没有那 5 行。
2. **新门禁的断言是空转的**：写成「表里的槽位 ⊆ 骨架里的【】」——
   **空集对任何集合都是子集，于是恒真**。
   **文档缺陷与门禁空转互相掩盖**：表空 → 断言真 → 门禁绿 → 没人去看表。

修法两步：

- §6.2 整段重写（一行不多一行不少：表头 + 分隔 + 5 行数据），实测表头出现次数 = 1；
- 门禁加 **非空前置**：先断言「真的取到了槽位行」（≥4 个），再谈子集关系。
  **反向验证补了两轮**：删掉数据行 → 非空前置红；槽位名改错一个字 → 「表与骨架不脱节」红
  并点名 `【主场景】`。

**教训（可推广）**：任何形如「A 是 B 的子集」「A 里每个都满足 P」的断言，
**只要 A 是从文件里解析出来的，就必须先断言 A 非空**——否则文件坏了它反而更绿。
顺手审计了全仓同类断言：其余几处的左值都是**字面量**（`{"reason","hint"}`、`{"A",..,"E"}`），
不会空，安全。

**这条路上我被自己的刀绊了三次**，一并记下免得再犯：
① `re.findall` 带捕获组时只返回**组**，我把它当整行写，于是数据行丢了表格语法；
② 用 `"---"` 切小节，结果命中了表格分隔行 `|---|---|---|`，把数据行切掉；
③ 断言写成子集式，空集恒真。
三条的共同点：**从文本里切表/切节时，不要用会命中文档自身语法的分隔符，也不要用带捕获组的
`findall` 当「取整行」，更不要写会空转的断言。** 都是我自己写检查工具时犯的，不是被测对象的问题。

### 待实施（本次只登记，未改代码）

**以下三条都只差一个决策（要花积分 / 要真样本），不是不知道怎么做，所以连命令一起写在这里：**

- **图形规则 6 项补实测**（扫描切片 / 结构图 / 模糊 / 线条 / 纹理 / 符号化；花积分）。
  落点已在 §一·丁 的选项库，缺的是「真出过图并通过」这半句。做法：按探索模式跑 2–3 张，
  验收走常规 `postcheck.py <图> --track A --top --text ok --note 'A/<风格>/<主题>/<主标题>'`，
  pass 后把 §一·丁 的「尚未实测」清单减掉对应项，并在 §六 家族落一条可复用填空。
  **不要**只把实测状态改成「已实测」而不出图——那正是这一轮修掉的那类错。
- **`description` 余量只有 94 B**（门禁口径 1906/2000）。这是**约束不是待办**：
  没有非加不可的触发词时什么都别做；真要加，**先砍再加**，
  砍源已有量化依据（触发词表 555 B 实测非承重），但砍之前要重评一次不对称风险。
- **B 轨台账为 0 条**（判据齐备、流程齐备，只是没有记录）。从下一张真 B 成品起：
  `postcheck.py <图> --track B --text ok --note 'B/<主题>/…'`，**判据是台账里出现 `track=B`**——
  出现不了就是记账缺陷（去查，别猜）。**历史的 0 条追不回来，也不该用合成图去补**：
  合成图只能证明记账通路通，证明不了生产覆盖。
...
- ~~**`runs.csv` 的 `note` 字段约定为 `A/<视觉风格>/<主题>/<主标题>`**~~　**1.22.1 已做**：
  约定已写进 `SKILL.md` §3 步 7；`log_report.py` 现在会报 **`note` 空值率**（不摆出来缺口只会靠盘点发现），
  并按 `note` 分批次。**没有**走「加 `--style` 列」那条路——实测 `note` 已能承担批次键与风格键，
  动表结构（含老机器的迁移路径）是白付的复杂度。
- ~~扫描切片、结构图、模糊、线条、纹理、符号化 **六种图形规则仍无模板落点**~~
  **1.22.1 纠正：这句话本身是错的**（见 §5 P1-6 副产品）。§一·丁 的选项库逐条给了这 11 条规则的
  「建立主体的用法」与「能表达的含义」——**那就是落点**。真正缺的是**实测记录**，动作见 1.22.1 待实施。
- ~~**提示词归档进流程**~~　**1.22.1 已做**：`postcheck.py --prompt <txt>` 验收时顺带把原文收进
  `scripts/_prompts_archive/`（按「风格__主题__主标题」落名），三条纪律钉在 `test_prompt_archive`。
- **`runs.csv` 加批次号**，并让 `log_report.py` 同时出「按行 / 按批次」两榜。
- **`note` 空值率进报告**（现 44% 空，`paper_warm_hint` 只有 24% 可归因到风格），
  让缺口自己浮出来，而不是靠下次盘点发现。
