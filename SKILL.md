...
description: >-
  ...
  调用 scripts/*.py 完成）。元提示词库 + 出图工作流，三条赛道——赛道 A 竖版概念海报（v5.4，
...
> **WorkBuddy 专属 Skill**：出图走 WorkBuddy 的 ImageGen（文生图 / 图生图，模型在会话内切换），
> 填槽与验收由 Agent 调用 `scripts/*.py` 完成。脱离 WorkBuddy，脚本能单跑，但闭环不成立。
...
1. **先报积分**：ImageGen 单张约 5–10 积分。出图前必须在回复里说清这次预计消耗（N 张 × 5–10），批量出图（≥5 张）先确认。
