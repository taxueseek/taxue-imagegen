...
**豆包/即梦环境不要用上表**(那里走 2K)。完整 2K 尺寸表已下沉到 `references/jimeng-env.md` #9.0
(或 `jimeng.py --sizes`)；`jimeng.py` 按比例自动映射，**无需手动算像素**。
...
| 类型 A **横版**(H 系 3:2 限色平印人物：巨字横带+短句块 ／ 复古宣传画 2.5:1 四色木刻) | `poster-horizontal-series.md`：#一–#五 H 系(骨架+十一槽位表)，#六 复古宣传画(骨架+槽位表+两版差异)；**要精确复现 H 系某张**再读 `h-series-prompts.md`(按风格节读，禁止整读) | `poster-v5.md` 整份(**竖版骨架，两种横版都不通用**) |
...
| 追溯某个版本改了什么 | `CHANGELOG.md` 对应版本节 | **出图不读**——全文约 107 KB，只在需要版本史时按版本节读 |
...
> 阈值不是拍出来的：每条 blocker 的**触发率**都在 202 张已归档成品上实测过；数据与复算见
> `sub-skills/verify/evidence.md` #4(`python3 scripts/calibrate_thresholds.py ~/Pictures/WorkBuddy`)。改判据先跑它。

**三条例外各自的处置入口**(都不阻断交付，但**都不许静默算 pass**)：

- **水印**：默认 `--wm auto`——先识别、命中才动手、去后复检(三分支与 amp / R² 双条件闸门见子技能 #3)；
  `--dewm` 等价 `--wm force`，`--wm off` 关闭。**平台署名水印**(右下角「AI生成 / WORKBUDDY」)不在这三条
  分支内，走 `scripts/dewm_imprint.py`(须同系列多张一起跑，坑 36)。`dewm` 报 `conf<0.1` / `k̂≈0` 是它在说
  「这张图我解不了」——不要 `--no-guard` 硬解、也不要手工 inpaint 硬框，改用**分材质填充**(坑 35)。
- **泛黄/纸白偏暖**(坑 33)：白底母题会被模型当材质色渲染，`R-B` 稳定偏暖且**提示词压不下来** →
  `[paper_warm]` + **pending**，处置是 `scripts/paper_white.py`(免费、只动纸白像素；**重生改不了偏色**)。
- **`top_noise`**(坑 27)：**诊断值不是 blocker**(阈值 6 在 13/13 张真实海报上全触发)；要看顶部是否真被
  画脏，用 `top_lf_std` / `top_dark%` + 目检裁片。

---

## 6. 云端后处理(可选项，先建议后启用)

> **完整编排、操作映射与六条调用纪律见 `references/cloud-postprocess.md`**(触发：云端修复、
> 后处理、抠图、去疑难水印、修老照片)。本节只留「何时该路由过去」的三条判据：

- **可选项，不默认启用**：与重生并列的另一条补救路；默认路径仍是三档评审的原结论
  (可修不重生、blocker 定向重生)，**绝不由 Agent 静默发起**。
- **授权边界**：用户主动要求修图/增强/抠图视为已授权；Agent 评审后建议的**必须获用户同意**，
  话术说清三点——① 修什么(哪个操作、动画面哪一块)；② 代价(重生 = 再扣 5–10 积分且已验收
  部分作废；云端只动指定区域但**计费口径未实测**)；③ 无论走哪条**修完都要重跑 `postcheck.py`**。
- **去水印双路由**：本技能 ImageGen 出的图走本地 `dewm*`(免费、逐位保护画面)；外来图 /
  水印压复杂图形 / dewm 全族救不回 → 官方 `erase` 云端重绘(**周边会被重画，调用前必须告知**)。
...
| references(按需读) | 何时用 |
|---|---|
| `references/poster-v5.md` | 类型 A 操作层：v5.4 模板全文、三个段落级变体(#一·乙 穿插 / #一·丙 横版 16:9 / #一·丁 图形规则系统)、填空规则、满铺·孤置(#五)、已验证风格台账(#六) |
| `references/poster-horizontal-series.md` | 类型 A 的两支**横版**母版(与竖版 poster-v5 互不通用)：#一–#五 H 系 3:2 限色平印(骨架 + 11 槽位 + 11 风格表)；#六 复古宣传画 2.5:1 四色木刻 |
| `references/h-series-prompts.md` | H 系 11 份成品提示词原文(43KB，**禁止整读**，按风格节读)：精确复现某张、或查某槽位的长写法 |
...
| `references/crowd-100-faces-prompt-v1.md` · `references/crowd-100-faces-prompt-v2.md` | 主题五高密度百相图完整提示词(脸复制 / 年龄配额修复实验；v2 是当前版) |
...
| `references/cloud-postprocess.md` | #6 完整版：云端操作映射、去水印双路由、官方六条调用纪律 |
| `references/jimeng-env.md` | #9 完整版：适配边界表、**#9.0 即梦 2K 尺寸表**、S0–S6 平台检测信号链 |
| `sub-skills/verify/SKILL.md` | **出图验收子技能**：三档评审口径 / 水印自动政策 / 缺陷修复路由 / 各类型验收重点 |
| `sub-skills/verify/evidence.md` | 验收阈值的**实测依据与样本量**(改阈值先读它，复算走 `calibrate_thresholds.py`) |

**scripts(出图只碰这些)**——加粗的是默认入口，其余按需：

| 文件 | 何时用 |
|---|---|
| `scripts/fill_meta.py` | **出稿默认入口**：A/B/C/E 全走它，组装完内嵌 preflight；`--list` 查槽位/主题，`--manpu` 切满铺型 |
| `scripts/postcheck.py` | **验收默认入口**：一次调用完成量测 + 文字带 2x 裁片 + 水印自动识别 + runs.csv 记账；`--track A–E`，E 加 `--expect-cells N`；verdict 三值 = 退出码 blocker 1 / pending 3 / pass 0(工具错误 4，**不是 blocker**) |
| `scripts/preflight.py` | 出图前静态检查(37 个坑中 11 个可文本拦截 + 残留槽位，认 `{}` 与【】)；fill_meta 已内嵌，手写提示词时单独跑 |
| `scripts/measure.py` | 单独量测 / `--grid` 出对比拼图；`grid_metrics()` 是类型 E 网格结构判定 |
| `scripts/jimeng.py` | 豆包/即梦出稿：只换算尺寸，prompt 与 fill_meta 逐字节一致；`--sizes` 查尺寸 |
| `scripts/wm_auto.py` | 水印自动识别与条件去除：amp + R² 双条件闸门 → skip/remove/manual；默认只探测 |
| `scripts/dewm_v10.py` | **默认单版去水印**：v9 管线 + 平底自适应融合(`--no-fuse` 关)+ 可解性门控(`--no-guard` 关，坑 26) |
| `scripts/pick_wm.py` | 疑难图选版：v6–v9 按「残留 + 结构损伤」选优，输出 `_clean/` 绝不覆盖原图(坑 28/29) |
| `scripts/dewm_imprint.py` | **平台署名水印**(右下角「AI生成 / WORKBUDDY」)：dewm 全族对它必报「解不了」、rmwm_light 掩膜退化，故单开本脚本，**须 N 张同系列一起跑**(坑 36) |
| `scripts/rmwm_light.py` · `scripts/rmwm.py` · `scripts/metric_flat.py` | 亮字修复(**覆盖率守卫**；含文字的图会啃笔画，坑 34/35，用前先 `--check`)/ 暗字 inpaint 对照 / 平底残影 RMS(坑 22) |
| `scripts/audit_wm.py` · `scripts/dewm_io.py` | 去水印质量审计(残留 + `--ref` 结构损伤 + 三联目检，坑 30)/ 共享 IO 层(覆盖守卫默认落 `_clean/` + 中文路径安全读写) |
| `scripts/dewm.py` · `scripts/dewm_v7.py` · `scripts/dewm_v8.py` · `scripts/dewm_v9.py` · `scripts/dewm_v11.py` · `scripts/dewm_v12.py` · `scripts/dewm_v13.py` · `scripts/dewm2.py` | 旧版与实验版(v6 反解 / v7 inpaint / v8 自适应 / v9 锚点对齐 / v11 暗区白残留 / v12 水印挪位 / v13 Wiener 融合 / Qwen 无模板)，同时是 pick_wm 的候选池；单版实测均不优于 v10，非默认 |
| `scripts/paper_white.py` | **纸白归正 + 背景去斑**(坑 33，确定性免费)：白底图 R-B≥3 先跑它再考虑重生，`--dry-run` 只看报告 |
| `scripts/explore.py` · `scripts/build_storyboard.py` | 探索模式(CSV 驱动批量出稿 + settle 改名校对)/ 类型 D 出稿(改 SCENES 生成 9 条；`build_storyboard_ink.py` 是旧命令转发壳) |
| `scripts/calibrate_thresholds.py` | 换模型 / 改判据前先跑：用已归档成品重算各判据触发率 |
| `scripts/bench_dewm_align.py` · `scripts/bench_dewm.py` · `scripts/probe_k_bias.py` | 改去水印代码必跑的合成基准：`TAXUE_BENCH_IMGS="a.png:b.png" python3 …` |
| `scripts/_env.py` · `scripts/_log.py` · `scripts/log_report.py` | 依赖自检(缺 numpy/PIL/cv2 时说人话 + 两条修复路径，退出码 2)/ 本地使用日志(`scripts/logs/usage.jsonl`，只落本地、不进发布仓)/ **读日志出「下一步该改什么」**(改文档前先看它) |
| `scripts/test_regressions.py` · `scripts/test_jimeng.py` · `scripts/run_tests.sh` · `.github/workflows/validate.yml` | 断言式回归入口(断言按域拆在 `scripts/tests/`)/ jimeng 回归(43 项，不依赖 numpy/PIL)/ 总测试门禁(本地与 CI 共用)/ CI 编排(装 `scripts/requirements.txt` 后跑总门禁) |

---

## 9. 豆包/即梦环境适配(jimeng.py)

> **完整适配边界、2K 尺寸表(#9.0)与 S0–S6 平台检测信号链见 `references/jimeng-env.md`**
> (触发：在豆包/即梦里跑本技能)。
