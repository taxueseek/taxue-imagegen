...
- **v1.22.2** (2026-09-23): **A measurement-driven performance pass** — the largest pure-Python cost on the default verification path now runs in C: `measure.metrics` used to expand a 256-wide thumbnail into 98,000 RGB tuples and then make a dozen passes of `min()`/`max()` over them; it now uses `ImageChops` plus histograms plus `ImageStat.Stat(im, mask)`. **126 ms → 31 ms per call**, verified field-by-field against the previous implementation on **304 archived outputs with a maximum error of 0.0**; `postcheck --wm off` is 38% faster and the default `--wm auto` 26%. **The always-loaded SKILL.md shrinks 33,234 → 30,056 B (−9.6%)**: the Jimeng 2K size table moves out of SKILL.md into `references/jimeng-env.md` #9.0 (the table used to sit on the always-paid surface while the on-demand file pointed back at it — the exact inversion the layering rule exists to prevent), #6/#9 bodies become pointers, and the #8 index loses 1.1 KB; notation is unified (`§` → `#`, fullwidth parens → halfwidth, **prose only** — fenced templates are never touched). Five **class** defects fixed: `dewm_v11`'s "1−α too small" guard read an already-clipped α and so **never fired**; the alpha template box was not clamped to the canvas, so degenerate sizes (1600×100, 1×1) crashed the whole verification with `ValueError`/`cv2.error`; `sync_release.sh` would publish the validated prompt archive (`--exclude='_prompts'` cannot match `_prompts_archive`); `dewm2`/`rmwm` reported success when every image failed; `rmwm_light` turned an unreadable image into a traceback with exit code 1 (= "blocker"). Assertions 288 → 317, green on **Python 3.9 / 3.13 / 3.14**. See [CHANGELOG.md](./CHANGELOG.md).
- **v1.22.1** (2026-09-23): (Three more follow-ups: `paper_white._box_mean`'s window was off-centre by one cell, measurable via impulse response; `fill_meta B --theme 百相` was printing methodology notes and a 23-row metrics table instead of the prompt, whose real text lives in a separate file; and Type E decoded the same image twice — it now reuses the already-open image, verified field-identical across 40 real deliverables.) **Adds a local usage log** (`scripts/_log.py` + `scripts/log_report.py`) — every script invocation appends one line (which script, exit code, duration, host, Python version), and `log_report.py` turns that into "what to change next": call frequency / crash rate / p50·p95 latency / environment profile / **scripts that were never called** / the most frequent `runs.csv` codes. Local-only, never shipped, no argument values or absolute paths recorded, silently skipped if unwritable; concurrent appends use a single `O_APPEND` write (12 processes, 92 lines, zero corrupt). **Previously every pitfall edited the docs; now the data lands first and the docs are written once a pattern shows up.** Same batch fixes six *class-level* defects: `--out <source dir>` silently overwriting the original (dewm2/rmwm/rmwm_light/dewm_imprint each hand-rolled their own path handling instead of calling the guard already in the library); the `_clean/` redirect target never being created (`dewm_v10.py a.png --out <srcdir>` crashed with `FileNotFoundError`); tool failures being reported as blockers (an uncaught exception exits 1, and 1 means blocker — the agent would burn 5–10 credits regenerating; now exit code 4); `--help` being silently ignored while doing the work (`build_storyboard_ink.py --help` really wrote nine prompt files); the same diagnostic code appearing twice on one row (Type E carried a permanent false diagnosis and `runs.csv` kept two `grid_diag` entries); and `dewm_imprint.py` missing the `_env` dependency check while silently turning 16-bit input into pure white and still printing `[ok]`. Eight further single-point fixes include preflight eating the last character of an unterminated final line, `jimeng` reporting a size that disagreed with its own output when `--set 比例` was repeated, and `fill_meta` using the same exit code for a failed `--out` write as for "preflight found a blocker". Third round of resident-surface de-duplication, plus four factual corrections (pitfall count 35→37, pitfalls 104→114 KB, CHANGELOG 34→49 KB, `_env.py` previously unindexed). Assertion functions 32→37, regression checks 200→221; **green on Python 3.9.6 / 3.13.12 / 3.14.7**. See [CHANGELOG.md](./CHANGELOG.md).
- **v1.22.0** (2026-09-23): **An inventory-driven backfill** — the skill had 18 catalogued styles while the `runs.csv` ledger still held **13 styles that had passed in practice but were never written down** (the `note` field never recorded the style, so they could only be reverse-engineered from filenames); all 13 are now recorded as `poster-v5.md #六·戊`, alongside three blacklisted high-failure styles. Adds Type A's second and third paragraph-level variants: **#一·丙 landscape 16:9 with two text levels** (including the hard-won rule that the subtitle must never sit bottom-right — that is exactly where the platform imprint lands, and `dewm_imprint.py` needs a whole series at once) and **#一·丁 graphic-rule systems** (eleven rule families, primary/auxiliary pairings, three hard rules). **Corrects a size conclusion**: 16:9 can be requested directly as `1536x864` with no cropping (the earlier "no exact 16:9; use 3:2 and crop" was disproved by measurement). See [CHANGELOG.md](./CHANGELOG.md).
- **v1.21.2** (2026-09-23): **New pitfall 37** — on a RISO two-colour, high-contrast poster with large flat white areas, `postcheck --wm auto` reporting `SUSPECT(|amp|=1.37)` is a **false alarm** (1.37/255 ≈ 0.5%, invisible to the eye; `dewm_v10.py`'s own visibility gate had already declined it). The verdict rule is now two-step: `|amp| < 2.0` together with flat-area std ≈ 0 and no glyph in the magnified crop → **close it out**, and do not run `pick_wm` just to clear the pending state (its four-way pick reverse-solves noise out of a clean image). A counter-example boundary is recorded too: synthetic halftone swatches take the other branch (high amp + low R²), so reproducing it that way yields the opposite conclusion. See [CHANGELOG.md](./CHANGELOG.md).
